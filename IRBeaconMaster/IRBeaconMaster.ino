/*
Arduino code for the master IR beacon in the robot navigational system.
The modulation ASCII character is transmitted continuously and periodically, with
a 250 msec delay between transmissions. After each transmission a 1 msec pulse
is sent to pin D9 to send a trigger to the first slave down the beacon chain.
...
void setup() {
  int i;
  i = 2; // set D2 through D13 to be digital outputs
  while (i <= 13) {
    pinMode(i,OUTPUT);
    i++;
  }
...
// or output frequency = 16MHz / ( 2 * prescale factor * (OCR2A+1))
  ...
  modulationState = 0;  // Wait initial 250 msec delay
  serialState = 0;
}
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (serialState == 0) {
      ...
      if (commandChar == '?') {
        Serial.print("Infrared Beacon Master: Character ");
